  ...
  double t)
{
  ...
  Q_v = a_vx * a_vx * PI * PI * v_x * mu * cos(a_vx * PI * x / L) * pow(L, -0.2e1) + 0.4e1 / 0.3e1 * a_vy * a_vy * PI * PI * v_y * mu * sin(a_vy * PI * y / L) * pow(L, -0.2e1) + a_vz * a_vz * PI * PI * v_z * mu * sin(a_vz * PI * z / L) * pow(L, -0.2e1) + a_rhox * PI * rho_x * U * V * cos(a_rhox * PI * x / L) / L - a_rhoy * PI * rho_y * V * V * sin(a_rhoy * PI * y / L) / L + a_rhoz * PI * rho_z * V * W * cos(a_rhoz * PI * z / L) / L - a_vx * PI * v_x * RHO * U * sin(a_vx * PI * x / L) / L + a_vz * PI * v_z * RHO * W * cos(a_vz * PI * z / L) / L + a_rhot * PI * rho_t * V * cos(a_rhot * PI * t / L) / L + a_vt * PI * v_t * RHO * cos(a_vt * PI * t / L) / L + a_py * PI * p_y * cos(a_py * PI * y / L) / L + (a_ux * u_x * cos(a_ux * PI * x / L) + 0.2e1 * a_vy * v_y * cos(a_vy * PI * y / L) - a_wz * w_z * sin(a_wz * PI * z / L)) * PI * RHO * V / L;
  return(Q_v);
}
