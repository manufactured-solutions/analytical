  ...
  double *theta_e_N2)
{
  ...
  e_vib_eq_N2 = theta_v_N2 * R / M_N / (exp(theta_v_N2 / T) - 0.1e1) / 0.2e1;
  ...
  Q_eV = e_elec_N * a_rho_N_x * PI * rho_N_x * U * cos(a_rho_N_x * PI * x / L) / L - (e_vib_N2 + e_elec_N2) * a_rho_N2_x * PI * rho_N2_x * U * sin(a_rho_N2_x * PI * x / L) / L - e_vib_N2 * a_Tvx * PI * Tv_x * theta_v_N2 * RHO_N2 * U * sin(a_Tvx * PI * x / L) / L * pow(TV, -0.2e1) + e_elec_N * a_ux * PI * u_x * RHO_N * cos(a_ux * PI * x / L) / L + (e_vib_N2 + e_elec_N2) * a_ux * PI * u_x * RHO_N2 * cos(a_ux * PI * x / L) / L - e_elec_N * a_rho_N_t * PI * rho_N_t * sin(a_rho_N_t * PI * t / L) / L + (e_vib_N2 + e_elec_N2) * a_rho_N2_t * PI * rho_N2_t * cos(a_rho_N2_t * PI * t / L) / L - 0.2e1 * e_vib_N2 * e_vib_N2 * M_N * a_Tvx * PI * Tv_x * RHO_N2 * U * sin(a_Tvx * PI * x / L) / R * pow(TV, -0.2e1) / L + (theta_v_N2 * R + 0.2e1 * M_N * e_vib_N2) * e_vib_N2 * a_Tvt * PI * Tv_t * RHO_N2 * cos(a_Tvt * PI * t / L) / R * pow(TV, -0.2e1) / L + De_elecN_Dx * RHO_N * U + De_elecN2_Dx * RHO_N2 * U + De_elecN_Dt * RHO_N + De_elecN2_Dt * RHO_N2 - w_dot_V;
  return(Q_eV);
}
