...
\rho(x,t) &= \rho_{\text{N}}(x,t)+\rho_{\text{N}_2}(x,t).
          %&= \rho_{\text{N}0} + \rho_{\text{N}_2 0} +\rho_{\text{N}x} \sin\left(\frac{a_{  \rho \text{N} x }\pi x}{L}\right) + \rho_{\text{N}_2 x} \cos\left(\frac{a_{ \rho \text{N}_2 x } \pi x}{L}\right) +\rho_{\text{N}t} \cos\left( \dfrac{ a_{ \rho \text{N} t}  \pi t}{L}\right)+\rho_{\text{N}_2 t} \sin\left( \dfrac{ a_{ \rho \text{N}_2 t}\pi t}{L}\right).
\end{split}
\end{equation}
...
%The resulting source terms for the 1D transient Euler flow with Nitrogen dissociation in thermal equilibrium described by Equations (\ref{eq:euler01}) are obtained through symbolic manipulation using the software Maple and are presented in the following sections.
