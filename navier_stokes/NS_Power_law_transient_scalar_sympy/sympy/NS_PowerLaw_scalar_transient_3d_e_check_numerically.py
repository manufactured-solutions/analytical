...
# Auxiliary relations -------------------------------------------------------------------------------------------
var("R,gamma,T_ref,mu_ref,Pr,alpha,beta")
T = p/(rho*R);
e = R*T/(gamma-1);
e_t = e+(u*u+v*v+w*w)/2;
...
LAMBDA = (alpha- Rational(2,3) )*Mu;
LAMBDA =LAMBDA.subs(parameters).evalf();
...
W*a_pz*p_z*sin(pi*a_pz*z/L) - V*a_py*p_y*cos(pi*a_py*y/L))/(L*(1 - gamma)) + LAMBDA*pi**2*(U*u_x*a_ux**2*sin(pi*a_ux*x/L) +
V*v_y*a_vy**2*sin(pi*a_vy*y/L) + W*w_z*a_wz**2*cos(pi*a_wz*z/L) - 2*a_ux*a_vy*u_x*v_y*cos(pi*a_ux*x/L)*cos(pi*a_vy*y/L) +
2*a_ux*a_wz*u_x*w_z*cos(pi*a_ux*x/L)*sin(pi*a_wz*z/L) + 2*a_vy*a_wz*v_y*w_z*cos(pi*a_vy*y/L)*sin(pi*a_wz*z/L) - a_ux**2*u_x**2*cos(pi*a_ux*x/L)**2 -
a_vy**2*v_y**2*cos(pi*a_vy*y/L)**2 - a_wz**2*w_z**2*sin(pi*a_wz*z/L)**2)/L**2 + Mu*pi**2*(U*(u_y*a_uy**2*cos(pi*a_uy*y/L) +
u_z*a_uz**2*cos(pi*a_uz*z/L) + 2*u_x*a_ux**2*sin(pi*a_ux*x/L)) + V*(v_x*a_vx**2*cos(pi*a_vx*x/L) + v_z*a_vz**2*sin(pi*a_vz*z/L) +
2*v_y*a_vy**2*sin(pi*a_vy*y/L)) + W*(w_x*a_wx**2*sin(pi*a_wx*x/L) + w_y*a_wy**2*sin(pi*a_wy*y/L) + 2*w_z*a_wz**2*cos(pi*a_wz*z/L)) -
2*a_uy*a_vx*u_y*v_x*sin(pi*a_uy*y/L)*sin(pi*a_vx*x/L) - 2*a_vz*a_wy*v_z*w_y*cos(pi*a_vz*z/L)*cos(pi*a_wy*y/L) +
2*a_uz*a_wx*u_z*w_x*cos(pi*a_wx*x/L)*sin(pi*a_uz*z/L) - a_uy**2*u_y**2*sin(pi*a_uy*y/L)**2 - a_uz**2*u_z**2*sin(pi*a_uz*z/L)**2 -
a_vx**2*v_x**2*sin(pi*a_vx*x/L)**2 - a_vz**2*v_z**2*cos(pi*a_vz*z/L)**2 - a_wx**2*w_x**2*cos(pi*a_wx*x/L)**2 - a_wy**2*w_y**2*cos(pi*a_wy*y/L)**2 -
2*a_ux**2*u_x**2*cos(pi*a_ux*x/L)**2 - 2*a_vy**2*v_y**2*cos(pi*a_vy*y/L)**2 - 2*a_wz**2*w_z**2*sin(pi*a_wz*z/L)**2)/L**2 + pi*DMu_Dx*U*(-(2/3 -
alpha)*(a_wz*w_z*sin(pi*a_wz*z/L) - a_ux*u_x*cos(pi*a_ux*x/L) - a_vy*v_y*cos(pi*a_vy*y/L)) - 2*a_ux*u_x*cos(pi*a_ux*x/L))/L +
pi*DMu_Dx*V*(a_uy*u_y*sin(pi*a_uy*y/L) + a_vx*v_x*sin(pi*a_vx*x/L))/L + pi*DMu_Dx*W*(a_uz*u_z*sin(pi*a_uz*z/L) - a_wx*w_x*cos(pi*a_wx*x/L))/L +
pi*DMu_Dy*U*(a_uy*u_y*sin(pi*a_uy*y/L) + a_vx*v_x*sin(pi*a_vx*x/L))/L + pi*DMu_Dy*V*(-(2/3 - alpha)*(a_wz*w_z*sin(pi*a_wz*z/L) -
a_ux*u_x*cos(pi*a_ux*x/L) - a_vy*v_y*cos(pi*a_vy*y/L)) - 2*a_vy*v_y*cos(pi*a_vy*y/L))/L + pi*DMu_Dy*W*(-a_vz*v_z*cos(pi*a_vz*z/L) -
a_wy*w_y*cos(pi*a_wy*y/L))/L + pi*DMu_Dz*U*(a_uz*u_z*sin(pi*a_uz*z/L) - a_wx*w_x*cos(pi*a_wx*x/L))/L + pi*DMu_Dz*V*(-a_vz*v_z*cos(pi*a_vz*z/L) -
a_wy*w_y*cos(pi*a_wy*y/L))/L + pi*DMu_Dz*W*(-(2/3 - alpha)*(a_wz*w_z*sin(pi*a_wz*z/L) - a_ux*u_x*cos(pi*a_ux*x/L) - a_vy*v_y*cos(pi*a_vy*y/L)) +
2*a_wz*w_z*sin(pi*a_wz*z/L))/L + pi*E_t*Rho*(a_ux*u_x*cos(pi*a_ux*x/L) + a_vy*v_y*cos(pi*a_vy*y/L) - a_wz*w_z*sin(pi*a_wz*z/L))/L +
pi*E_t*a_rhot*rho_t*cos(pi*a_rhot*t/Lt)/Lt + pi*V*a_py*p_y*cos(pi*a_py*y/L)/L + pi*a_pt*p_t*sin(pi*a_pt*t/Lt)/(Lt*(1 - gamma)) +
pi*a_rhox*rho_x*(E_t*U + P*U/(Rho*(1 - gamma)))*cos(pi*a_rhox*x/L)/L + pi*a_rhoy*rho_y*(-E_t*V - P*V/(Rho*(1 - gamma)))*sin(pi*a_rhoy*y/L)/L +
pi*a_rhoz*rho_z*(E_t*W + P*W/(Rho*(1 - gamma)))*cos(pi*a_rhoz*z/L)/L + kappa*pi**2*(p_x*a_px**2*cos(pi*a_px*x/L) + p_y*a_py**2*sin(pi*a_py*y/L) +
p_z*a_pz**2*cos(pi*a_pz*z/L))/(L**2*R*Rho) + kappa*pi**2*(-2*a_px*a_rhox*p_x*rho_x*cos(pi*a_rhox*x/L)*sin(pi*a_px*x/L) -
2*a_py*a_rhoy*p_y*rho_y*cos(pi*a_py*y/L)*sin(pi*a_rhoy*y/L) - 2*a_pz*a_rhoz*p_z*rho_z*cos(pi*a_rhoz*z/L)*sin(pi*a_pz*z/L))/(L**2*R*Rho**2) -
pi*U*a_px*p_x*sin(pi*a_px*x/L)/L - pi*W*a_pz*p_z*sin(pi*a_pz*z/L)/L + P*kappa*pi**2*(-2*a_rhox**2*rho_x**2*cos(pi*a_rhox*x/L)**2 -
2*a_rhoy**2*rho_y**2*sin(pi*a_rhoy*y/L)**2 - 2*a_rhoz**2*rho_z**2*cos(pi*a_rhoz*z/L)**2)/(L**2*R*Rho**3) +
P*kappa*pi**2*(-rho_x*a_rhox**2*sin(pi*a_rhox*x/L) - rho_y*a_rhoy**2*cos(pi*a_rhoy*y/L) - rho_z*a_rhoz**2*sin(pi*a_rhoz*z/L))/(L**2*R*Rho**2) +
pi*P*a_rhot*rho_t*cos(pi*a_rhot*t/Lt)/(Lt*Rho*(1 - gamma)) );
