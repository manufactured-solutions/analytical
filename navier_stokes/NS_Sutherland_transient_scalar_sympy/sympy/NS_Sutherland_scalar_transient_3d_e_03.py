...
execfile("NS_Sutherland_scalar_transient_3d_e_02.py")
...
#Q=Q.subs(kappa,k);
#Q=Q.subs(Mu,mu);
#Q=Q.subs(-R*gamma*DMu_Dx/(1 - gamma), diff(k,x))
#Q=Q.subs(-R*gamma*DMu_Dy/(1 - gamma), diff(k,y))
#Q=Q.subs(-R*gamma*DMu_Dz/(1 - gamma), diff(k,z))
...
Res=Res.subs( diff(k,x),-R*gamma*DMu_Dx/(1 - gamma)/Pr)
Res=Res.subs( diff(k,y),-R*gamma*DMu_Dy/(1 - gamma)/Pr)
Res=Res.subs( diff(k,z),-R*gamma*DMu_Dz/(1 - gamma)/Pr)
Res=Res.subs(diff(mu,x),DMu_Dx);
Res=Res.subs(diff(mu,y),DMu_Dy);
Res=Res.subs(diff(mu,z),DMu_Dz);
Res=Res.subs(mu,Mu);
Res=Res.subs(k,kappa);
Res=expand(Res)
...
#pi * t / Lt));
  ...
  #Q=Q.subs(Mu,mu);
  #Q=Q.subs(kappa,k);
  #Q=Q.subs(diff(k,x),-R*gamma*diff(mu,x)/(1 - gamma)/Pr)
  #Q=Q.subs(diff(k,y),-R*gamma*diff(mu,y)/(1 - gamma)/Pr)
  #Q=Q.subs(diff(k,z),-R*gamma*diff(mu,z)/(1 - gamma)/Pr)
  #Q=Q.subs(DMu_Dx,diff(mu,x));
  #Q=Q.subs(DMu_Dy,diff(mu,y));
  #Q=Q.subs(DMu_Dz,diff(mu,z));
  ##
