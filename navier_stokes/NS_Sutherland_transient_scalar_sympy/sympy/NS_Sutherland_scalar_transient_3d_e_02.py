...
# Q3 ------------------------------------------------------------------
var("Mu, DMu_Dx, DMu_Dy, DMu_Dz,kappa,Pr")
#diff(kappa,x)=-R*gamma*D(mu(x, y, z, t), x)/(1 - gamma)
...
Q4n=Q4n.subs(diff(k,x), -R*gamma*DMu_Dx/(1 - gamma)/Pr)
Q4n=Q4n.subs(diff(k,y), -R*gamma*DMu_Dy/(1 - gamma)/Pr)
Q4n=Q4n.subs(diff(k,z), -R*gamma*DMu_Dz/(1 - gamma)/Pr)
Q4n=Q4n.subs(k,kappa)

Q4n=expand(Q4n)
Q4n=collect(Q4n,[2*kappa*pi**2*P/(L**2*R*Rho**3)],exact=True)
Q4n=collect(Q4n,[kappa*pi**2*P/(L**2*R*Rho**2)],exact=True)

#print(Q4n)
A1= ( P*kappa*pi**2*(-2*a_rhox**2*rho_x**2*cos(pi*a_rhox*x/L)**2 - 2*a_rhoy**2*rho_y**2*sin(pi*a_rhoy*y/L)**2 -
...
- rho_z*a_rhoz**2*sin(pi*a_rhoz*z/L))/(L**2*R*Rho**2)  )
A2=Q4n-A1


A2=collect(A2,[kappa*pi**2*L*R*Rho])
#termo novo
#A3= ( + pi*DMu_Dy*a_py*gamma*p_y*cos(pi*a_py*y/L)/(L*Rho*(1 - gamma)) - pi*DMu_Dx*a_px*gamma*p_x*sin(pi*a_px*x/L)/(L*Rho*(1 - gamma)) -
#pi*DMu_Dz*a_pz*gamma*p_z*sin(pi*a_pz*z/L)/(L*Rho*(1 - gamma)) + pi*DMu_Dy*P*a_rhoy*gamma*rho_y*sin(pi*a_rhoy*y/L)/(L*Rho**2*(1 - gamma)) -
#pi*DMu_Dx*P*a_rhox*gamma*rho_x*cos(pi*a_rhox*x/L)/(L*Rho**2*(1 - gamma)) - pi*DMu_Dz*P*a_rhoz*gamma*rho_z*cos(pi*a_rhoz*z/L)/(L*Rho**2*(1 - gamma)) 
#)
