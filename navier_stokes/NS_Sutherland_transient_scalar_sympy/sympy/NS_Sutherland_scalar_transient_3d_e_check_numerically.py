...
# Auxiliary relations -------------------------------------------------------------------------------------------
var("R,A_mu,B_mu,k,gamma,T_ref,mu_ref")
T = p/(rho*R);
e = R*T/(gamma-1);
e_t = e+(u*u+v*v+w*w)/2;
mu = A_mu*T**(3/2)/(T+B_mu);
...
k*pi**2*(p_x*a_px**2*cos(pi*a_px*x/L) + p_y*a_py**2*sin(pi*a_py*y/L) + p_z*a_pz**2*cos(pi*a_pz*z/L))/(L**2*R*Rho) +
k*pi**2*(-2*a_px*a_rhox*p_x*rho_x*cos(pi*a_rhox*x/L)*sin(pi*a_px*x/L) - 2*a_py*a_rhoy*p_y*rho_y*cos(pi*a_py*y/L)*sin(pi*a_rhoy*y/L) -
...
P*k*pi**2*(-2*a_rhox**2*rho_x**2*cos(pi*a_rhox*x/L)**2 - 2*a_rhoy**2*rho_y**2*sin(pi*a_rhoy*y/L)**2 -
2*a_rhoz**2*rho_z**2*cos(pi*a_rhoz*z/L)**2)/(L**2*R*Rho**3) + P*k*pi**2*(-rho_x*a_rhox**2*sin(pi*a_rhox*x/L) - rho_y*a_rhoy**2*cos(pi*a_rhoy*y/L) -
