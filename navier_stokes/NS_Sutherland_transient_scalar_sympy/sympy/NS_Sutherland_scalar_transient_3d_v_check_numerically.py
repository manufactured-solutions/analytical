...
#3D transient Navier-Stokes equation - velocity v:
diff(rho*v,t) + diff(rho*u*v,x) + diff(rho*v*v,y) + diff(rho*v*w,z) + diff(p, y)==diff(tau_yx, x) + diff(tau_yy, y) + diff(tau_yz, z);
  


# Auxiliary relations -------------------------------------------------------------------------------------------
var("R,A_mu,B_mu,k,gamma,E_t,mu_ref,T_ref")
T = p/(rho*R);
e = R*T/(gamma-1);
e_t = e+(u*u+v*v+w*w)/2;
mu = A_mu*T**(3/2)/(T+B_mu); #it was commented in the other files
...
# Velocity-v equation, writen as differential operator:
Lo = diff(rho*v,t) + diff(rho*u*v,x) + diff(rho*v*v,y) + diff(rho*v*w,z) + diff(p, y)-( diff(tau_yx, x) + diff(tau_yy, y) + diff(tau_yz, z) );

L1 = + diff(rho*u*v,x) + diff(rho*v*v,y) + diff(rho*v*w,z);
L2 = + diff(p, y);
L3 = -( diff(tau_yx, x) + diff(tau_yy, y) + diff(tau_yz, z) );
L4 = diff(rho*v,t);
...
Qv_num=Qv.subs(parameters).evalf()  
...
Q_v= ( pi*DMu_Dx*(a_uy*u_y*sin(pi*a_uy*y/L) + a_vx*v_x*sin(pi*a_vx*x/L))/L + pi*DMu_Dy*(-4*a_vy*v_y*cos(pi*a_vy*y/L)/3 - 2*a_wz*w_z*sin(pi*a_wz*z/L)/3
+ 2*a_ux*u_x*cos(pi*a_ux*x/L)/3)/L + pi*DMu_Dz*(-a_vz*v_z*cos(pi*a_vz*z/L) - a_wy*w_y*cos(pi*a_wy*y/L))/L + Mu*pi**2*(v_x*a_vx**2*cos(pi*a_vx*x/L) +
v_z*a_vz**2*sin(pi*a_vz*z/L) + 4*v_y*a_vy**2*sin(pi*a_vy*y/L)/3)/L**2 + pi*Rho*V*(a_ux*u_x*cos(pi*a_ux*x/L) - a_wz*w_z*sin(pi*a_wz*z/L) +
2*a_vy*v_y*cos(pi*a_vy*y/L))/L + pi*a_py*p_y*cos(pi*a_py*y/L)/L + pi*Rho*a_vt*v_t*cos(pi*a_vt*t/Lt)/Lt + pi*V*a_rhot*rho_t*cos(pi*a_rhot*t/Lt)/Lt -
pi*a_rhoy*rho_y*V**2*sin(pi*a_rhoy*y/L)/L + pi*Rho*W*a_vz*v_z*cos(pi*a_vz*z/L)/L + pi*U*V*a_rhox*rho_x*cos(pi*a_rhox*x/L)/L +
pi*V*W*a_rhoz*rho_z*cos(pi*a_rhoz*z/L)/L - pi*Rho*U*a_vx*v_x*sin(pi*a_vx*x/L)/L
 );


Q_v_num=Q_v.subs(parameters).evalf()  
  
  
print 'Q_v_num-Qv_num=',Q_v_num-Qv_num 
