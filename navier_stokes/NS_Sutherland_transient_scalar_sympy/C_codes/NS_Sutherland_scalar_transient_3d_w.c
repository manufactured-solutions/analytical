...
#include "NS_Sutherland_scalar_transient_3d_w.h"
#include <math.h>
...
double Q_w_viscous(double DMu_Dx, double DMu_Dy, double DMu_Dz, double L, double Mu, double a_ux, double a_uz, double a_vy, double a_vz, double a_wx, double a_wy, double a_wz, double u_x, double u_z, double v_y, double v_z, double w_x, double w_y, double w_z, double x, double y, double z) {
  return M_PI*DMu_Dx*(a_uz*u_z*sin(M_PI*a_uz*z/L) - a_wx*w_x*cos(M_PI*a_wx*x/L))/L + M_PI*DMu_Dy*(-a_vz*v_z*cos(M_PI*a_vz*z/L) - a_wy*w_y*cos(M_PI*a_wy*y/L))/L + M_PI*DMu_Dz*(2*a_ux*u_x*cos(M_PI*a_ux*x/L)/3 + 2*a_vy*v_y*cos(M_PI*a_vy*y/L)/3 + 4*a_wz*w_z*sin(M_PI*a_wz*z/L)/3)/L + Mu*pow(M_PI,2)*(w_x*pow(a_wx,2)*sin(M_PI*a_wx*x/L) + w_y*pow(a_wy,2)*sin(M_PI*a_wy*y/L) + 4*w_z*pow(a_wz,2)*cos(M_PI*a_wz*z/L)/3)/pow(L,2);
}
