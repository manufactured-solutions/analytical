...
#include "NS_Sutherland_scalar_transient_3d_u.h"
#include <math.h>
...
double Q_u_viscous(double DMu_Dx, double DMu_Dy, double DMu_Dz, double L, double Mu, double a_ux, double a_uy, double a_uz, double a_vx, double a_vy, double a_wx, double a_wz, double u_x, double u_y, double u_z, double v_x, double v_y, double w_x, double w_z, double x, double y, double z) {
  return M_PI*DMu_Dx*(-4*a_ux*u_x*cos(M_PI*a_ux*x/L)/3 - 2*a_wz*w_z*sin(M_PI*a_wz*z/L)/3 + 2*a_vy*v_y*cos(M_PI*a_vy*y/L)/3)/L + M_PI*DMu_Dy*(a_uy*u_y*sin(M_PI*a_uy*y/L) + a_vx*v_x*sin(M_PI*a_vx*x/L))/L + M_PI*DMu_Dz*(a_uz*u_z*sin(M_PI*a_uz*z/L) - a_wx*w_x*cos(M_PI*a_wx*x/L))/L + Mu*pow(M_PI,2)*(u_y*pow(a_uy,2)*cos(M_PI*a_uy*y/L) + u_z*pow(a_uz,2)*cos(M_PI*a_uz*z/L) + 4*u_x*pow(a_ux,2)*sin(M_PI*a_ux*x/L)/3)/pow(L,2);
}
