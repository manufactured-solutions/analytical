  ...
  double *theta_e_N2)
{
  ...
  Q_E = 0.3e1 / 0.2e1 * a_ux * PI * u_x * RHO * U * U * cos(a_ux * PI * x / L) / L + (h0_N + e_elec_N) * a_rho_N_x * PI * rho_N_x * U * cos(a_rho_N_x * PI * x / L) / L - (h0_N2 + e_elec_N2 + e_vib_N2) * a_rho_N2_x * PI * rho_N2_x * U * sin(a_rho_N2_x * PI * x / L) / L + (0.10e2 * RHO_N + 0.7e1 * RHO_N2) * a_ux * PI * u_x * R * T * cos(a_ux * PI * x / L) / L / M_N / 0.4e1 + (h0_N + e_elec_N) * a_ux * PI * u_x * RHO_N * cos(a_ux * PI * x / L) / L + (h0_N2 + e_elec_N2 + e_vib_N2) * a_ux * PI * u_x * RHO_N2 * cos(a_ux * PI * x / L) / L - (0.10e2 * RHO_N + 0.7e1 * RHO_N2) * a_Tx * PI * T_x * R * U * sin(a_Tx * PI * x / L) / L / M_N / 0.4e1 - e_vib_N2 * a_Tvx * PI * Tv_x * theta_v_N2 * RHO_N2 * U * sin(a_Tvx * PI * x / L) / L * pow(TV, -0.2e1) - 0.2e1 * e_vib_N2 * e_vib_N2 * a_Tvx * PI * Tv_x * M_N * RHO_N2 * U * sin(a_Tvx * PI * x / L) / R * pow(TV, -0.2e1) / L - (-a_rho_N_x * rho_N_x * cos(a_rho_N_x * PI * x / L) + a_rho_N2_x * rho_N2_x * sin(a_rho_N2_x * PI * x / L)) * PI * pow(U, 0.3e1) / L / 0.2e1 - (-0.10e2 * a_rho_N_x * rho_N_x * cos(a_rho_N_x * PI * x / L) + 0.7e1 * a_rho_N2_x * rho_N2_x * sin(a_rho_N2_x * PI * x / L)) * PI * R * U * T / L / M_N / 0.4e1 + De_elecN_Dx * RHO_N * U + De_elecN2_Dx * RHO_N2 * U;
  return(Q_E);
}
